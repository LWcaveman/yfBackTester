...
class PortfolioBacktesterSwing:
    def __init__(
        self,
        strategy,
        ...
        stale_bars: int = 12,
        stop_buffer_pct: float = 0.008 # 0.8% buffer below the signal candle low
    ):
    ...
    def run(self, ticker_data: dict[str, pd.DataFrame]) -> dict:
        ...
        for i in range(1, len(all_dates)):
            ...
            for sym, pos in list(open_positions.items()):
                ...
                if curr_row["high"] >= target_main:
                    reason_label = f"TARGET_{self.target_r}R"
                    t = self._record_exit(pos, target_main, curr_date, reason_label, entry_p, r_val, shares)
                    trades.append(t)
                    pending_settlements.append((curr_date, target_main * shares))
                    closed_this_bar.append(sym)
                    continue
            ...
            if available_slots > 0 and settled_cash > 10.0 and is_market_bullish:
                ...
                for sym, _, curr_bar, prev_bar in candidates[:available_slots]:
                    ...
                    signal_low = prev_bar.get("planned_stop", entry_p * 0.98) 
                    stop_p = signal_low * (1.0 - self.stop_buffer_pct)
                    risk_per_share = max(entry_p - stop_p, 0.01)
