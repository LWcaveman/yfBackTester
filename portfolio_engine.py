"""
Unified Institutional Portfolio Backtester Engine
Simulates realistic bar-by-bar multi-ticker execution with cash settlement,
regime filters, risk management, and earnings defense.
"""
...
class PortfolioBacktester:
    ...
    def run(self, ticker_data: dict[str, pd.DataFrame]) -> dict:
        ...
        for i in range(1, len(all_dates)):
            ...
            # Process settled cash (T+1 settlement)
            new_settlements = []
            for avail_date, amt in pending_settlements:
                if curr_date >= avail_date:
                    settled_cash += amt
                else:
                    new_settlements.append((avail_date, amt))
            ...
            for sym, pos in list(open_positions.items()):
                ...
                # 2. Stop Loss / Breakeven breach check at Low
                ...
                # 3. Target hit check at High
                if curr_row["high"] >= target_p:
                    reason_label = "TARGET_2R" if self.target_r == 2.0 else f"TARGET_{self.target_r:.1f}R"
                    t = self._record_exit(pos, target_p, curr_date, reason_label, entry_p, r_val, shares)
                    trades.append(t)
                    pending_settlements.append((curr_date, target_p * shares))
                    closed_this_bar.append(sym)
                    continue

                # 4. Breakeven ratchet: move stop to entry once +1R is hit
                if curr_row["high"] >= r1_p and stop_p < entry_p:
                    pos["stop_loss"] = entry_p

                # 5. Stale trade time-stop
                ...
                # 6. Earnings defense exit: close position on session prior to earnings
            ...
            open_val = sum(
                signals[s].loc[curr_date]["close"] * p["shares"]
                for s, p in open_positions.items()
                if curr_date in signals[s].index
            )
            ...
            if available_slots > 0 and settled_cash > 10.0 and is_market_bullish:
                ...
                # Sort candidates by tightest pullback to the 20 EMA
                candidates.sort(key=lambda x: x[1])

                for sym, _, curr_bar, prev_bar in candidates[:available_slots]:
                    entry_p = curr_bar["open"]

                    # Planned stop with optional buffer below candle low
                    raw_stop = prev_bar.get("planned_stop", entry_p * 0.98)
                    buffered_stop = raw_stop * (1.0 - self.stop_buffer_pct)
                    dist_raw = entry_p - buffered_stop
                    dist_floor = entry_p * self.min_risk_pct

                    risk_per_share = max(dist_raw, dist_floor)
                    stop_p = entry_p - risk_per_share

                    # Position sizing: risk_pct of current equity, capped by slot capital allocation
                    max_pos_cost = current_equity / self.max_positions
                    target_risk_dollars = current_equity * self.risk_pct
                    ...
                    if actual_shares <= 0 or (actual_shares * entry_p) < 5.0:
                        continue
                    ...
                    open_positions[sym] = {
                        ...
                        "target_main": round(entry_p + (self.target_r * risk_per_share), 2),
                        "shares": round(actual_shares, 4) if self.allow_fractional else actual_shares,
                        "bars_held": 0,
                        "highest_price": float(curr_bar["high"]),
                        "lowest_price": float(curr_bar["low"]),
                    }
    ...
    def _generate_portfolio_report(self, trades, equity_curve):
        if not trades:
            return {"total_trades": 0, "final_balance": self.starting_capital}
